...
`),
	...
	_ = cmd.MarkFlagCustom(cli.StripDash(cli.ConfigurationRefFlagName), "__riff_list_knative_configurations")
	cmd.Flags().StringVar(&opts.ServiceRef, cli.StripDash(cli.ServiceRefFlagName), "", "`name` of Knative service to update")
	_ = cmd.MarkFlagCustom(cli.StripDash(cli.ServiceRefFlagName), "__riff_list_knative_services")
