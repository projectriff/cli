...
func (opts *HandlerCreateOptions) Exec(ctx context.Context, c *cli.Config) error {
	...
	if opts.ApplicationRef != "" {
		handler.Spec.Build = &corev1alpha1.Build{
	...
	if opts.DryRun {
		...
		var err error
		handler, err = c.CoreRuntime().Handlers(opts.Namespace).Create(handler)
		if err != nil {
			return err
		}
	}
	...
	if opts.Tail {
		...
		err := race.Run(ctx, timeout,
			func(ctx context.Context) error {
				return k8s.WaitUntilReady(ctx, c.CoreRuntime().RESTClient(), "handlers", handler)
			},
			func(ctx context.Context) error {
				return c.Kail.CoreHandlerLogs(ctx, handler, cli.TailSinceCreateDefault, c.Stdout)
			},
		)
...
`),
		Example: strings.Join([]string{
			...
			fmt.Sprintf("%s core handler create my-func-handler %s my-func", c.Name, cli.FunctionRefFlagName),
			fmt.Sprintf("%s core handler create my-func-handler %s my-container", c.Name, cli.ContainerRefFlagName),
			fmt.Sprintf("%s core handler create my-image-handler %s registry.example.com/my-image:latest", c.Name, cli.ImageFlagName),
		}, "\n"),
