...
func (opts *HandlerTailOptions) Exec(ctx context.Context, c *cli.Config) error {
	handler, err := c.CoreRuntime().Handlers(opts.Namespace).Get(opts.Name, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	return c.Kail.CoreHandlerLogs(ctx, handler, since, c.Stdout)
}
...
`),
		Example: strings.Join([]string{
			fmt.Sprintf("%s core handler tail my-handler", c.Name),
			fmt.Sprintf("%s core handler tail my-handler %s 1h", c.Name, cli.SinceFlagName),
		}, "\n"),
