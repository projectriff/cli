...
func (opts *HandlerListOptions) Exec(ctx context.Context, c *cli.Config) error {
	handlers, err := c.CoreRuntime().Handlers(opts.Namespace).List(metav1.ListOptions{})
	if err != nil {
		return err
	}
...
`),
		Example: strings.Join([]string{
			...
			fmt.Sprintf("%s core handler list %s", c.Name, cli.AllNamespacesFlagName),
		}, "\n"),
...
func (opts *HandlerListOptions) printList(handlers *corev1alpha1.HandlerList, printOpts printers.PrintOptions) ([]metav1beta1.TableRow, error) {
...
func (opts *HandlerListOptions) print(handler *corev1alpha1.Handler, _ printers.PrintOptions) ([]metav1beta1.TableRow, error) {
	now := time.Now()
...
func (opts *HandlerListOptions) formatRef(handler *corev1alpha1.Handler) (string, string) {
