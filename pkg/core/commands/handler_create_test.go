...
`,
		...
		},
		{
			...
			Prepare: func(t *testing.T, ctx context.Context, c *cli.Config) (context.Context, error) {
				...
				c.Kail = kail
				kail.On("CoreHandlerLogs", mock.Anything, &corev1alpha1.Handler{
					ObjectMeta: metav1.ObjectMeta{
						Namespace: defaultNamespace,
						Name:      handlerName,
					},
...
To view status run: riff core handler list --namespace default
To continue watching logs run: riff core handler tail my-handler --namespace default
`,
			ShouldError: true,
