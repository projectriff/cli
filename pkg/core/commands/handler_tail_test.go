	...
	table := rifftesting.CommandTable{
		...
		},
		{
			...
			Prepare: func(t *testing.T, ctx context.Context, c *cli.Config) (context.Context, error) {
				...
				c.Kail = kail
				kail.On("CoreHandlerLogs", mock.Anything, handler, cli.TailSinceDefault, mock.Anything).Return(nil).Run(func(args mock.Arguments) {
...
`,
		},
		{
			...
			Prepare: func(t *testing.T, ctx context.Context, c *cli.Config) (context.Context, error) {
				kail := &kailtesting.Logger{}
				c.Kail = kail
				kail.On("CoreHandlerLogs", mock.Anything, handler, time.Hour, mock.Anything).Return(nil).Run(func(args mock.Arguments) {
					fmt.Fprintf(c.Stdout, "...log output...\n")
				})
				return ctx, nil
			},
...
`,
		...
		},
		{
			...
			Prepare: func(t *testing.T, ctx context.Context, c *cli.Config) (context.Context, error) {
				kail := &kailtesting.Logger{}
				c.Kail = kail
				kail.On("CoreHandlerLogs", mock.Anything, handler, cli.TailSinceDefault, mock.Anything).Return(fmt.Errorf("kail error"))
				return ctx, nil
			},
			CleanUp: func(t *testing.T, ctx context.Context, c *cli.Config) error {
				kail := c.Kail.(*kailtesting.Logger)
