...
func (opts *HandlerDeleteOptions) Exec(ctx context.Context, c *cli.Config) error {
	client := c.CoreRuntime().Handlers(opts.Namespace)
...
`),
		Example: strings.Join([]string{
			fmt.Sprintf("%s core handler delete my-handler", c.Name),
			fmt.Sprintf("%s core handler delete %s", c.Name, cli.AllFlagName),
		}, "\n"),
